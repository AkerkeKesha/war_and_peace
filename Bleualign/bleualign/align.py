...
from Bleualign.bleualign.gale_church import align_texts
import Bleualign.bleualign.score as bleu
from Bleualign.bleualign.utils import evaluate, finalevaluation
import io
import platform
...
class Aligner:
    ...
    def __init__(self, options):
        ...
        if not self.options['srctotarget'] and not self.options['targettosrc'] \
            ...
            raise ValueError(
                "ERROR: no translation available: BLEU scores can be computed between the source and target text, but this is not the intended usage of Bleualign and may result in poor performance! If you're *really* sure that this is what you want, set 'galechurch' for the options.")
    ...
    def mainloop(self):
        ...
        if multiprocessing_enabled and self.options['num_processes'] > 1:
            ...
            scorers = [AlignMultiprocessed(tasks, self.options, scores, self.log) for i in
                       range(self.options['num_processes'])]

            for p in scorers:
                p.start()

            # this function produces the alignment tasks for the consumers in scorers
            producer = multiprocessing.Process(target=tasks_producer, args=(
            tasks, num_tasks, (self.src, self.srctotarget, self.target, self.targettosrc, self.options),
            self.options['num_processes']))
            producer.start()
            ...
            for i, (sourcelist, targetlist, translist1, translist2) in enumerate(
                    collect_article(self.src, self.srctotarget, self.target, self.targettosrc, self.options)):
    ...
    def eval_sents(self, translist, targetlist):
        ...
        for testID, testSent in enumerate(translist):
            ...
            cooked_test["guess"] = [max(len(test_normalized) - k + 1, 0) for k in
                                    range(1, self.options['bleu_ngrams'] + 1)]
            ...
            for (refID, (reflen, refmaxcounts, refset)) in cooktarget:
                ...
                if ngrams_filtered:
                    ...
                    for ngram in ngrams_filtered:
                        cooked_test["correct"][self.options['bleu_ngrams'] - 1] += min(refmaxcounts[ngram],
                                                                                       counts[ngram])
                    ...
                    if score > 0:
                        ...
                        cooked_test2["guess"] = [max(cooked_test['reflen'] - k + 1, 0) for k in
                                                 range(1, self.options['bleu_ngrams'] + 1)]
                        logbleu = 0.0
                        for k in range(self.options['bleu_ngrams']):
    ...
    def pathfinder(self, translist, targetlist):
        ...
        # TODO: return matrix and do backtracking
    ...
    def gapfiller(self, sourcegap, targetgap, pregap, postgap, translist, targetlist):
        ...
        while sourcegap or targetgap:
            ...
            if sourcegap and targetgap:
                ...
                if "galechurch" in self.options['gapfillheuristics'] and (
                        max(len(targetgap), len(sourcegap)) < 4 or max(len(targetgap), len(sourcegap)) / min(
                        len(targetgap), len(sourcegap)) < 2):
                    tempsrcgap = []
        ...
        if not pregap in [i[0] for i in self.multialign]:
            if sourcegap:
                self.addtoAlignments(((sourcegap[0],), (None,)))
            elif targetgap:
                print(sourcegap, targetgap)
                self.addtoAlignments(((None,), (targetgap[0],)))
            else:
                self.addtoAlignments(pregap)
    ...
    def addtoAlignments(self, pair, aligntype=None):
        if not (pair[0] is not None and pair[1] is not None):
            return  # TODO: here it skips
        if aligntype:
    ...
    def print_alignment_statistics(self, source_len, target_len):
        ...
        if source_len and target_len:
            self.log("\n" + str(len(self.bleualign)) + ' out of ' + str(
                source_len) + ' source sentences aligned by BLEU ' + str(
                100 * len(self.bleualign) / float(source_len)) + '%', 2)
            self.log("after gap filling, " + str(multialignsrccount) + ' out of ' + str(
                source_len) + ' source sentences aligned ' + str(100 * multialignsrccount / float(source_len)) + '%', 2)
            self.log("after gap filling, " + str(multialigntargetcount) + ' out of ' + str(
                target_len) + ' target sentences aligned ' + str(100 * multialigntargetcount / float(target_len)) + '%',
                     2)
    ...
    def printout(self, sourcelist, translist, targetlist):
        ...
        pprint(self.multialign)
        # self.multialign = sorted(self.multialign,key=itemgetter(0))
        sentscores = {}
        lastsrc, lasttarget = 0, 0
        # multialign = [(((0,), (None,)), 'GAPFILLER'), (((1,), (0,)), 'BLEU')]
        # for j,(src,target) in enumerate([i[0] for i in multialign]):
        for j, (src, target) in enumerate([i[0] for i in self.multialign]):
            ...
            lastsrc = src[-1]
            lasttarget = target[-1] if target else ['']

            translations.append(' '.join([translist[ID] if ID is not None else '' for ID in src]))
            if self.options['factored']:
                ...
                sources.append(' '.join([sourcelist[ID] if ID is not None else '' for ID in src]))
                targets.append(' '.join([targetlist[ID] if ID is not None else '' for ID in target]))

            if self.options['filter'] == 'sentences':
                self.check_sentence_pair(j, sources[-1], translations[-1], targets[-1], sources_output[-1],
                                         targets_output[-1], sentscores)
